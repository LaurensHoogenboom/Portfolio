<script lang="ts">
	...
	let {
		...
		customRoute,
		customTitle
	}: { closeCallback: () => void; children: Snippet; itemTitle: string; itemTitleKey: string, customRoute?: string, customTitle?: string } = $props();

	let saving = $state(false);
	let errorMessage: string | undefined = $state();
</script>

<Dialog title={customTitle ?? `Edit ${itemTitle}`} {closeCallback}>
	<form
		method="post"
		action={customRoute ?? '?/update'}
